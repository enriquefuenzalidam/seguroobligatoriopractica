    ...
    <>
      {precioTablas.map((tabla, index) => (
        <div key={index} className=" my-20 mx-6">
          <h3 className="bg-sky-600 text-white text-center text-xl sm:text-xl md:text-2xl lg:text-2xl py-3 sm:py-3 md:py-4 lg:py-6 mb-6 font-bold uppercase mask-image-url mask-repeat-no mask-size-cover mask-position-center">{tabla.mes}</h3>
          <div className={` mx-auto inline-grid  ${(tabla.seguros.length > 1) ? 'grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2': 'grid-cols-1' } gap-10 justify-center `}>
            {tabla.seguros.map((seguro, seguroIndex) => (
              <div key={seguroIndex} className=" flex flex-col h-full w-96 border-solid border-2 border-neutral-100 rounded-md py-6 shadow-lg shadow-neutral-500">
